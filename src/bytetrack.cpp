...
static void generate_grids_and_stride(const int target_w, const int target_h, vector<int>& strides, vector<GridAndStride>& grid_strides)
{
    for (auto stride : strides)
    {
        ...
        for (int g1 = 0; g1 < num_grid_h; g1++)
        {
            ...
            {
                GridAndStride gas {g0,g1,stride};
...
static void qsort_descent_inplace(vector<Detection>& faceobjects, int left, int right)
{
    int i = left;
    int j = right;
...
static void qsort_descent_inplace(vector<Detection>& objects)
{
    if (objects.empty())
        return;
...
static void nms_sorted_bboxes(const vector<Detection>& faceobjects, vector<int>& picked, float nms_threshold)
{
    picked.clear();
...
static void generate_yolox_proposals(vector<GridAndStride> grid_strides, float* feat_blob, float prob_threshold, vector<Detection>& objects)
{
...
//
//static void decode_outputs(float* prob, vector<Detection>& objects, float scale, const int img_w, const int img_h) {
//        vector<Detection> proposals;
